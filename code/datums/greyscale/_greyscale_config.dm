...
/datum/greyscale_config/New()
	...
	var/static/regex/iris_gags_regex = regex(@"^modular_iris\/modules\/GAGS\/(?:[\w]+\/)*json_configs\/(?:[\w]+\/)*\w+\.json$") // IRIS EDIT ADDITION - Added recognition for the Iris GAGS folder
	if(findtext(string_json_config, "code/datums/greyscale/json_configs/") != 1 && nova_gags_regex.Find(string_json_config) != 1 && iris_gags_regex.Find(string_json_config) != 1) // NOVA EDIT CHANGE - ORIGINAL: if(findtext(string_json_config, "code/datums/greyscale/json_configs/") != 1) // IRIS EDIT CHANGE - added Iris modularization
		stack_trace("All greyscale json configuration files should be located within '/greyscale/json_configs/' or 'modular_nova/modules/GAGS/json_configs/' or 'modular_iris/modules/GAGS/json_configs/'.") // NOVA EDIT CHANGE - ORIGINAL: stack_trace("All greyscale json configuration files should be located within 'code/datums/greyscale/json_configs/'") // IRIS EDIT CHANGE - added Iris modularization
	if(!icon_file)
