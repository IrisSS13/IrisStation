...
/obj/item/mod/skin_applier/interact_with_atom(atom/attacked_atom, mob/living/user, params)
	if(!isatom(attacked_atom) || !(attacked_atom.type in subtypesof(/obj/item/mod/control)))
		return NONE
	var/obj/item/mod/control/mod = attacked_atom
	if(mod.active || mod.activating)
		balloon_alert(user, "unit active!")
		return ITEM_INTERACT_BLOCKING
	if(!universal && !(skin in mod.theme.variants))
		balloon_alert(user, "wrong theme for skin!")
		return ITEM_INTERACT_BLOCKING
	if(universal) // adds this skin to the variants of this suit
