...
/datum/ai_behavior/find_unwebbed_turf/perform(seconds_per_tick, datum/ai_controller/controller)
	...
	if (current_target && !(locate(/obj/structure/chosm/membrane) in current_target))
		// Already got a target
		return AI_BEHAVIOR_DELAY | AI_BEHAVIOR_FAILED

	controller.clear_blackboard_key(target_key)
	var/turf/our_turf = get_turf(spider)
	if (is_valid_membrane_turf(our_turf, spider))
	...
	for (var/turf/turf_in_view in oview(scan_range, our_turf))
		if (!is_valid_membrane_turf(turf_in_view, spider))
			continue
...
/datum/ai_behavior/find_unwebbed_turf/proc/is_valid_membrane_turf(turf/target_turf, mob/living/spider)
	if (locate(/obj/structure/chosm/membrane) in target_turf)
		return FALSE
